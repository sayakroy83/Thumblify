    ...
    <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
      ...
      {!loading && thumbnails.length === 0 && (
        <div className="text-center py-24">
          <h3 className="text-lg text-zinc-400 mt-2">No thumbnails yet</h3>
      ...
      {!loading && thumbnails.length > 0 && (
        <div className="columns-1 sm:columns-2 lg:columns-3 2xl:columns-4 gap-8">
          {thumbnails.map((thumb: IThumbnail)=>{
            ...
            return (
              <div key={thumb._id} onClick={()=> navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer 
              rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                ...
                <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                  {thumb.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-self-center text-sm text-zinc-400">
                      {thumb.isGenerating ? 'Generating...' : 'No image'}
                    </div>
                  )}

                  {thumb.isGenerating && <div className="absolute inset-0 bg-black/50 items-center justify-center 
                ...
                <div onClick={(e)=> e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1.5">
                  ...
                  <Link target="_blank" to={`/preview?thumbmail_url=${thumb.image_url}&title=${thumb.title}`}>
